...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", type=str, default="/home/chaoqi/projects/weights/temp/checkpoint/ppo_hh_jsd_0.1")
    ...
    model = AutoModelForCausalLMWithHydraValueHead.from_pretrained(ckpt_path).to(device) #load_model(model_path, weights_path, device)
    ...
    eval_prompts = eval_prompts[args.start_idx * 2:args.end_idx * 2]
    ...
    # if len(eval_dataloader) < args.start_idx:
    #     raise ValueError(f"start_idx {args.start_idx} is larger than the number of batches {len(eval_dataloader)}")
